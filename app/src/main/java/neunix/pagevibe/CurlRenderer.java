...
public class CurlRenderer implements GLSurfaceView.Renderer {
	...
	// Background fill color. Defaults to OPAQUE WHITE — this is the color
	// shown in any letterboxed margin around the page mesh and, crucially,
	// during the brief window before CurlView's explicit
	// setBackgroundColor() call reaches the render thread. Previously this
	// field had no explicit default, so it silently fell back to Java's
	// implicit int default of 0 (fully transparent BLACK) until the first
	// externally-set color arrived — that gap, plus onSurfaceCreated()
	// hardcoding black independently of this field, is what produced a
	// dark/muddy background instead of clean white.
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		// Uses mBackgroundColor (not a hardcoded literal) so there is no
		// code path in this class that can ever paint black/grey
		// regardless of surface-recreation timing (first launch, resume,
		// config change all re-trigger this callback).
		gl.glClearColor(Color.red(mBackgroundColor) / 255f,
				Color.green(mBackgroundColor) / 255f,
				Color.blue(mBackgroundColor) / 255f,
				Color.alpha(mBackgroundColor) / 255f);
