...
def find_project_dir():
    """Auto-detect project directory with a numbered selection menu."""
    cwd = os.getcwd()

    # Case 1: already inside a project folder
    if os.path.exists(os.path.join(cwd, "app", "build.gradle")):
        return cwd

    # Case 2: inside ApkBuilder root or PROJECTS_DIR exists
    if os.path.exists(PROJECTS_DIR):
        projects = [p for p in os.listdir(PROJECTS_DIR) 
                    if os.path.isdir(os.path.join(PROJECTS_DIR, p))]
        
        if projects:
            log(f"📦 {len(projects)} projects found:")
            # Display projects in a numbered list, each on a new line
            for i, name in enumerate(projects, start=1):
                print(f"  {i}. {name}")
            ...
            while True:
                ...
                try:
                    ...
                    if 0 <= idx < len(projects):
                        selected_path = os.path.join(PROJECTS_DIR, projects[idx])
                        log(f"✅ Selected: {projects[idx]}")
            ...
            log("⚠️ No projects found in the projects directory.")
    ...
    log("❌ Could not locate a valid project directory.")
    sys.exit(1)
...
def cmd_change_package():
    """Change Android package name, update file contents, and realign folders."""
    import re
    import shutil
    ...
    if os.path.exists(os.path.join(current_dir, "app", "src", "main", "AndroidManifest.xml")):
        ...
        if not choice.isdigit() or not (1 <= int(choice) <= len(projects)):
            ...
            return
        app_dir = os.path.join(PROJECTS_DIR, projects[int(choice) - 1])

    # --- 2. File Paths ---
    manifest_path = os.path.join(app_dir, "app", "src", "main", "AndroidManifest.xml")
    gradle_path = os.path.join(app_dir, "app", "build.gradle")
    java_root = os.path.join(app_dir, "app", "src", "main", "java")

    # --- 3. Detect Old Package Name ---
    if not os.path.exists(manifest_path):
        log("❌ Manifest not found. Is this a valid project?")
        return

    with open(manifest_path, "r", encoding="utf-8") as f:
        manifest_data = f.read()
    
    # Try to find package in manifest or namespace in build.gradle
    old_package = None
    match = re.search(r'package="([\w\.]+)"', manifest_data)
    if match:
        old_package = match.group(1)
    else:
        # Fallback: Check build.gradle for namespace
        with open(gradle_path, "r") as f:
            g_data = f.read()
            ns_match = re.search(r"namespace\s+['\"]([\w\.]+)['\"]", g_data)
            if ns_match: old_package = ns_match.group(1)

    if not old_package:
        log("❌ Could not detect old package name.")
        return

    log(f"🔍 Current package: {old_package}")
    new_package = input("🆕 Enter new package name: ").strip()
    if not re.match(r'^[a-zA-Z_][\w\.]+$', new_package):
        log("❌ Invalid format.")
        return

    # --- 4. Update Build Files (Gradle & Manifest) ---
    # Update Manifest
    manifest_data = manifest_data.replace(f'package="{old_package}"', f'package="{new_package}"')
    write_file(manifest_path, manifest_data)

    # Update Gradle (namespace and applicationId)
    if os.path.exists(gradle_path):
        with open(gradle_path, "r", encoding="utf-8") as f:
            gradle_data = f.read()
        gradle_data = re.sub(r'applicationId\s+["\']' + re.escape(old_package) + r'["\']', f'applicationId "{new_package}"', gradle_data)
        gradle_data = re.sub(r'namespace\s+["\']' + re.escape(old_package) + r'["\']', f'namespace "{new_package}"', gradle_data)
        write_file(gradle_path, gradle_data)
        log("📄 Updated build.gradle and AndroidManifest.xml")

    # --- 5. Move Folders and Update Source Code ---
    old_path = os.path.join(java_root, *old_package.split("."))
    new_path = os.path.join(java_root, *new_package.split("."))
    ...
    if os.path.exists(old_path):
        ...
        # Update the 'package ...' line inside every file
        for root, _, files in os.walk(new_path):
            for f_name in files:
                if f_name.endswith((".java", ".kt")):
                    file_path = os.path.join(root, f_name)
                    with open(file_path, "r", encoding="utf-8") as f:
                        content = f.read()
                    content = content.replace(f"package {old_package}", f"package {new_package}")
                    # Also handle imports of R class or BuildConfig if they exist
                    content = content.replace(f"import {old_package}.R", f"import {new_package}.R")
                    with open(file_path, "w", encoding="utf-8") as f:
                        f.write(content)
        log(f"📦 Folders realigned and source code updated.")
    else:
        log("⚠️ Source folder not found. Only build files were updated.")

    # --- 6. Update Config ---
    project_cfg_path = os.path.join(app_dir, "builder_config.json")
    updated_cfg = {
        "app_name": os.path.basename(app_dir),
        "package_name": new_package,
        "project_path": app_dir
    }
    save_json(project_cfg_path, updated_cfg)
    log("✅ Package change complete!")
