...
def download_latest_artifact():
    """
    Downloads latest artifact from GitHub Actions using `gh`.
    Extracts into outputs/ folder.
    """
    out_dir = os.path.join(os.getcwd(), OUTPUTS_DIR)
    os.makedirs(out_dir, exist_ok=True)

    print("[builder] 📦 Fetching latest artifact...")

    # Get latest run id
    try:
        # List runs and get the latest success for main.yml
        result = subprocess.run(
            "gh run list -L 5 --json databaseId,conclusion,status,artifacts",
        ...
        latest_success = None
        for run in runs:
            if run.get("conclusion") == "success" and run.get("artifacts") > 0:
                latest_success = run
                break

        if not latest_success:
            print("[builder] ⚠️ No successful runs with artifacts found.")
            return

        run_id = latest_success["databaseId"]

        # Download artifacts
        subprocess.run(f"gh run download {run_id} --dir {out_dir} --name app-release.apk", shell=True, check=True)

        print(f"[builder] ✅ Artifact downloaded to: {out_dir}")

    except Exception as e:
        print(f"[builder] ⚠️ Failed to download artifact: {e}")
        print("[builder] 🔗 You can manually download via GitHub Actions page.")
...
def cmd_init():
    ...
    user_name = input("Enter your username(eg.Rohit)") 
